...
int main() {
   ...
   sort(all(ar));
   for (int i = 0; i < n; i++) {
      if (ar[i].first >= k)break;
      int a = k - ar[i].first;
      int l = 0, r = n - 1;
      while (l < r)
      {
         // debug(l)
         if (ar[l].second == ar[i].second) {
            l++;
            continue;
         }
         if (ar[r].second == ar[i].second) {
            r--;
            continue;
         }
         if (ar[l].first + ar[r].first < a) l++;
         else if (ar[l].first + ar[r].first == a) {
            int a1 = ar[l].second, a2 = ar[r].second, a3 = ar[i].second;
